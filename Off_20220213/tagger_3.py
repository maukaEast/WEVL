...
#look at each songfile in mp3 list and make search criteria from name omitting "a", "the", &c.
	...
	print("Search parameters for "+file+" artist name: ",art_contents)
	print("Search parameters for "+file+" title name: ",tit_contents)
	if (list_contains(DBase['Artist'],art_contents)):
		print("Found some terms in 'Artist' column")
		#full_values = DBase[(DBase['Artist']==file) & (DBase['show_date']=="Dec 4. 2022 ")]
		#full_dict_art = full_values.to_dict()
		#print(full_dict)
		#episode_DB = pd.concat([episode_DB,full_values], axis=0,ignore_index=True)
	#print("Search parameters for "+file+" are:",tit_contents)
	if (list_contains(DBase['Title'],tit_contents)):
		print("Found some terms in 'Title' column")
		#full_values = DBase[(DBase['Title']==file) & (DBase['show_date']=="Dec 4. 2022 ")]
